let cityInput = document.getElementById('city_input'),
    ...
    api_key = '32aa59babe1cc4738c427f03fd88783d',
    ...
    hourlyForecastCard = document.querySelectorAll('.hourly-forecast'),
...
function getWeatherDetails(name, lat, lon, country, state) {
    ...
    fetch(FORECAST_API_URL).then(res => res.json()).then(data => {
        let hourlyForecastData = data.list;
        hourlyForecastCard[0].innerHTML = '';  // Clear previous data
        for (let i = 0; i <= 7; i++) {
            let hrForecastDate = new Date(hourlyForecastData[i].dt_txt);
            let hr = hrForecastDate.getHours();
            let a = 'PM';
            if (hr < 12) a = 'AM';
            if (hr == 0) hr = 12;
            if (hr > 12) hr = hr - 12;
            hourlyForecastCard[0].innerHTML += `
                <div class="card">
                    <p>${hr} ${a}</p>
                    <img src="https://openweathermap.org/img/wn/${hourlyForecastData[i].weather[0].icon}.png" alt="">
                    <p>${(hourlyForecastData[i].main.temp - 273.15).toFixed(2)}&deg;C</p>
                </div>
            `;
        }
